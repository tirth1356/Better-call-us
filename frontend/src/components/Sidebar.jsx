...
export default function Sidebar({ title = "Control Panel" }) {
  ...
  const getAgencyStatus = (id) => {
    if (risk === "HIGH" || risk === "SEVERE" || risk === "CRITICAL") return { color: "text-red-600", bg: "bg-red-50", border: "border-red-200", label: "ACTION REQUIRED" };
    if (risk === "MEDIUM") return { color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", label: "MONITORING" };
    return { color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", label: "ALL SAFE" };
  };
  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 h-full bg-[#FFF8F0] border-r border-[#C08552]/15 pt-8 pb-6 px-4 fixed left-0 top-0 bottom-0 z-40">
      <div className="mb-6 px-2">
        <p className="text-[10px] font-black tracking-[0.25em] uppercase text-[#4B2E2B]/40">Intelligence Node</p>
        <h2 className="text-xl font-headline font-black text-[#4B2E2B]">{title}</h2>
      </div>
